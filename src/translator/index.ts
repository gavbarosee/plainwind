...
import { parseNonEmptyClasses, extractImportant, extractPrefix, extractVariants, extractOpacity } from "./parser";
...
function translateSingleClass(cls: string): string {
  ...
  const { className: withoutImportant, isImportant } = extractImportant(baseClass);

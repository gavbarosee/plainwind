...
describe('Categorization Performance', () => {
  ...
  describe('groupTranslationsByCategory - large sets', () => {
    const large50Classes = Array(50)
      .fill(null)
      .map((_, i) => `class-${i}`);
    const large50Translations = Array(50)
      .fill(null)
      .map((_, i) => `translation ${i}`);

    bench('50 classes', () => {
      groupTranslationsByCategory(large50Classes, large50Translations);
    });

    const large100Classes = Array(100)
      .fill(null)
      .map((_, i) => `class-${i}`);
    const large100Translations = Array(100)
      .fill(null)
      .map((_, i) => `translation ${i}`);
  ...
  describe('Real-world scenarios', () => {
    ...
    const buttonTranslations = [
      ...
      'rounded corners',
      'on hover: darker blue background',
      'when active: even darker blue background',
      'transition colors',
    ];

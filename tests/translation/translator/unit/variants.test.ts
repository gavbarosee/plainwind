...
} from '@src/core/translation/engine/variants';
import {
...
describe('VARIANT_DESCRIPTIONS', () => {
  it.each(VARIANT_INTERACTION_CASES)(
    'interaction: %s -> %s',
    (name, key, expected) => {
  ...
  it.each(VARIANT_RESPONSIVE_CASES)(
    'responsive: %s contains %s',
    (name, key, expected) => {
      expect(VARIANT_DESCRIPTIONS[key]).toContain(expected);
    }
  );

  it.each(VARIANT_DARK_MODE_CASES)(
    'dark mode: %s -> %s',
    (name, key, expected) => {
      expect(VARIANT_DESCRIPTIONS[key]).toBe(expected);
    }
  ...
  it.each(VARIANT_PSEUDO_ELEMENT_CASES)(
    'pseudo-element: %s -> %s',
    (name, key, expected) => {
      expect(VARIANT_DESCRIPTIONS[key]).toBe(expected);
    }
...
describe('applyVariants', () => {
  it.each(APPLY_VARIANTS_CASES)(
    'applies variants: %s + %s -> %s',

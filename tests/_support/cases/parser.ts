...
export const EXTRACT_VARIANTS_CASES: Array<
  [string, { variants: string[]; baseClass: string }]
> = [
  ['hover:bg-blue-500', { variants: ['hover'], baseClass: 'bg-blue-500' }],
  [
    'md:hover:focus:text-red-500',
    { variants: ['md', 'hover', 'focus'], baseClass: 'text-red-500' },
  ],
  ['flex', { variants: [], baseClass: 'flex' }],
  [
    'dark:[--my-var:value]',
    { variants: ['dark'], baseClass: '[--my-var:value]' },
  ],
  [
    'hover:[content:var(--value)]',
    { variants: ['hover'], baseClass: '[content:var(--value)]' },
  ],
  [
    'sm:md:lg:[clip-path:circle(50%)]',
    { variants: ['sm', 'md', 'lg'], baseClass: '[clip-path:circle(50%)]' },
  ],
  [
    'group-hover:peer-focus:bg-white',
    { variants: ['group-hover', 'peer-focus'], baseClass: 'bg-white' },
  ],
  ['', { variants: [], baseClass: '' }],
  [
    '[&:nth-child(3)]:block',
    { variants: ['[&:nth-child(3)]'], baseClass: 'block' },
  ],
];

// Complex variant extraction with bracket nesting
export const COMPLEX_VARIANT_EXTRACTION_CASES: Array<
...
export const EXTRACT_IMPORTANT_CASES: Array<
  [string, { className: string; isImportant: boolean }]
> = [
  ['bg-white!', { className: 'bg-white', isImportant: true }],
  ['bg-white', { className: 'bg-white', isImportant: false }],
  [
    'hover:bg-blue-500/50!',
    { className: 'hover:bg-blue-500/50', isImportant: true },
  ],
...
// Prefix extraction
export const EXTRACT_PREFIX_CASES: Array<
  [string, { className: string; prefix: string }]
> = [
...
// Opacity extraction
export const OPACITY_EXTRACTION_EDGE_CASES: Array<
